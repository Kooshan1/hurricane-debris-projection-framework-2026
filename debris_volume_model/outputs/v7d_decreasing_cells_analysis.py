"""
Investigate cells where v7d predicted debris volume DECREASES across the 2020 -> 2030 -> 2040
horizon, and identify which input-feature changes drive the decrease.

Methodology:
  1. For each storm in {ike, fema33, fema36}, load the per-cell v7d predictions for 2020/2030/2040.
...
FILE_YEARS = [2019, 2030, 2040]      # disk
...
def load_pred(storm, file_year):
    p = PRED_DIR / storm / str(file_year) / "V14_physics_v7d_predictions.shp"
...
def analyse_storm(storm):
    ...
    p20 = load_pred(storm, 2019)
    p30 = load_pred(storm, 2030)
    p40 = load_pred(storm, 2040)
    df = p20.merge(p30, on="FID", how="outer").merge(p40, on="FID", how="outer")

    # Drop NaNs
    df = df.dropna(subset=["pred_2019", "pred_2030", "pred_2040"]).reset_index(drop=True)
    df["d_30_20"] = df["pred_2030"] - df["pred_2019"]
    df["d_40_30"] = df["pred_2040"] - df["pred_2030"]
    df["d_total"] = df["pred_2040"] - df["pred_2019"]
    ...
    # Load inputs for 2019 and 2040
    in20 = load_input(storm, 2019)
    ...
    dec = df_full[(df_full["d_total"] < -10) & (df_full["pred_2019"] > 1)]
    inc = df_full[(df_full["d_total"] > 10) & (df_full["pred_2019"] > 1)]
    ...
    df_full[["FID", "pred_2019", "pred_2030", "pred_2040",
...
def main():
    ...
    for storm in STORMS:
        ...
        n_dec_active = ((df["d_total"] < -10) & (df["pred_2019"] > 1)).sum()
