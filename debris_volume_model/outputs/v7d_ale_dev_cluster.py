"""
Accumulated Local Effects (ALE) -- Apley & Zhu (2020), JRSS-B -- for v7d.

Motivation
----------
PDP for DT in v7d is -59 m^3 (p5 -> p95), contradicting the physical prior
...
Output: ALE curves for DT, DM, DO in run_v7d_hazard_interactions/evaluation/.
Also writes a CSV summarising the p5->p95 ALE slope alongside the
matching PDP slope from v7d_pdp_signs.csv for direct comparison.
"""
...
# Existing PDP slopes for the same features (from v7d_pdp_signs.csv) so
# the comparison table is one-shot rather than requiring a join.
PDP_SLOPES = {"DT": -59.353, "DM": 50.678, "DO": 131.140}


# ---------------------------------------------------------------------------
# Load model, scaler, manifest, test split (matches evaluate_v7plus.py)
# ---------------------------------------------------------------------------
with open(MAN) as f:
    m = json.load(f)
...
# Calibration factor -- same convention as evaluate_v7plus.py
with open(EVAL_DIR / "evaluation_summary.json") as f:
    c_factor = float(json.load(f)["calibration_factor"])
print(f"Loaded v7d (best epoch {ckpt['epoch']}), calibration c = {c_factor:.4f}")
...
fig.suptitle("v7d ALE for the dev cluster (free branch) -- on-manifold uncentred-PDP correction",
             fontsize=10)
