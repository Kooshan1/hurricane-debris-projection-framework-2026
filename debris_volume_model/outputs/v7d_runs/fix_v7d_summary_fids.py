"""
Fix the FID indexing in all 9 v7d per-tract result_summary CSVs.

Bug recap:
  - During the v7d Monte-Carlo, each per-sample DataFrame inherited
...
Fix:
  - Use the position-to-FID mapping (`v7d_position_to_fid.csv`) produced by
    `diagnose_v7d_fid.py` on the remote, which captured the exact
  ...
  - For each result_summary_*_v7d_seeds0-999.csv:
      * Read the CSV
...
RUNS_DIR = Path("debris_volume_model/outputs/v7d_runs")
MAPPING_CSV = RUNS_DIR / "v7d_position_to_fid.csv"
...
def main():
    ...
    assert m["v7d_csv_index_value"].nunique() == 100, "Mapping has duplicate v7d indices"
    ...
    print("Mapping is valid: 100 unique v7d indices -> 100 unique true tract FIDs in [1..101]\\{34}")

    mapping = dict(zip(m["v7d_csv_index_value"], m["true_tract_FID"]))
    ...
    for storm, year in SCENARIOS:
        csv_path = RUNS_DIR / f"result_summary_{storm}_{year}_v7d_seeds0-999.csv"
        ...
        # Sanity: first col should be exactly the 100 v7d index values
