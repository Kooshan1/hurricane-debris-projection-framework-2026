"""
Aggregate v7d Monte-Carlo results into paper-ready summaries.

For each of the 9 scenarios (3 storms x 3 years), we have:
  - result_summary_<storm>_<year>_v7d_seeds0-999.csv  (100 tracts x 1000 samples)
    Per-sample CL_fire_stations ratio per census tract
  - result_network_<storm>_<year>_v7d_seeds0-999.csv  (58813 links x 1000 samples)
    Per-sample road_closure (0/1) per network link

This script produces:
  1. v7d_tract_summary.csv  (n_tracts x [scenario] x [stat])
       Per-tract mean / std / SEM / 95% CI for each scenario
  2. v7d_link_summary.csv   (n_links x [scenario] x [stat])
       Per-link closure-rate p / std / SEM for each scenario
  3. v7d_county_aggregate.csv
...
RDIR = Path("debris_volume_model/outputs/v7d_runs")
...
DISPLAY_YEAR = {2019: 2020, 2030: 2030, 2040: 2040}
...
def aggregate_per_tract():
    ...
    for storm, year in SCENARIOS:
        f = RDIR / f"result_summary_{storm}_{year}_v7d_seeds0-999.csv"
    ...
    out_path = RDIR / "v7d_tract_summary.csv"
    out.to_csv(out_path)
...
def aggregate_per_link():
    ...
    rows = {}
    for storm, year in SCENARIOS:
        f = RDIR / f"result_network_{storm}_{year}_v7d_seeds0-999.csv"
        df = pd.read_csv(f, index_col=0)
        sample_cols = [c for c in df.columns if c.startswith("sample_")]
        arr = df[sample_cols].to_numpy(dtype=np.float32)
    ...
    out_path = RDIR / "v7d_link_summary.csv"
    out.to_csv(out_path)
    print(f"\nWrote {out_path} ({out.shape})")
...
def county_aggregate():
    ...
    out = pd.DataFrame(rows)
    out_path = RDIR / "v7d_county_aggregate.csv"
