"""
Convergence diagnostic for v7d Monte-Carlo, v2 (improved).
...
def parse_args():
    ...
    p.add_argument("--results-dir", type=str,
                   default="debris_volume_model/outputs/v7d_runs")
...
def main():
    ...
    print("=" * 80)
    summary_rows = []
    for f in sorted(rdir.glob("result_summary_*_v7d_*.csv")):
    ...
    pd.DataFrame(summary_rows).to_csv(rdir / "v7d_convergence_summary_tracts.csv", index=False)
    ...
    print("=" * 80)
    network_rows = []
    for f in sorted(rdir.glob("result_network_*_v7d_*.csv")):
    ...
    pd.DataFrame(network_rows).to_csv(rdir / "v7d_convergence_summary_network.csv", index=False)
