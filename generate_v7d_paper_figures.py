"""
Regenerate ALL paper figures using v7d Monte-Carlo + v7d per-cell predictions.

Reads from:
  - debris_volume_model/outputs/v7d_runs/result_summary_*_v7d_seeds0-999.csv  (CLR)
  - debris_volume_model/outputs/v7d_runs/result_network_*_v7d_seeds0-999.csv  (NetworkCondition)
  - outputs/final_debris_volume_output/<storm>/<year>/V14_physics_v7d_predictions.shp  (pred_m3)
  - outputs/hazard_rasters/<storm>/<year>/<variable>.tif                              (max_surge_depth, identical)

Writes to:
  - outputs/figure/generated_maps_v7d/                                                 (all 36 maps + legends)
  - debris_volume_model/outputs/v7d_paper_figures/config_plots_v7d_generated.yaml      (the config used)
...
OUTPUT_MAPS  = "./outputs/figure/generated_maps_v7d"
OUTPUT_LEGS  = "./outputs/figure/generated_maps_v7d/legends"
...
DATA_SOURCES = {
    "parameter_shapefile": {
        # v7d per-cell predictions
        "input_path_template": "./outputs/final_debris_volume_output/{hurricane_name}/{year}/V14_physics_v7d_predictions.shp",
    ...
    "clr": {
        # v7d 1000-sample MC
        "input_path_template": "./debris_volume_model/outputs/v7d_runs/result_summary_{hurricane_name}_{year}_v7d_seeds0-999.csv",
        "variables": ["CLR"],
    },
    "network_condition": {
        ...
        "input_path_template": "./debris_volume_model/outputs/v7d_runs/result_network_{hurricane_name}_{year}_v7d_seeds0-999.csv",
...
def make_plot_def(plot_type: str, hurricane: str, year: int, mode: str,
                  variable: str = None, baseline_year: int = 2019,
...
def build_plot_definitions() -> list:
    ...
    For each (plot_type) x (storm in {ike, fema33, fema36}):
      - 2019: baseline plot
      - 2030, 2040: difference (or percent_change for hazard) vs that storm's 2019
    ...
    for plot_type, variable, future_mode in plot_specs:
        for storm in storms:
            # Baseline (2019)
            defs.append(make_plot_def(plot_type, storm, 2019, "baseline",
                                       variable=variable))
            # Future years (difference vs 2019)
            for year in [2030, 2040]:
                defs.append(make_plot_def(plot_type, storm, year, future_mode,
                                           variable=variable, baseline_year=2019))
    return defs
...
def main():
    ...
    out_config_dir = Path("debris_volume_model/outputs/v7d_paper_figures")
    out_config_dir.mkdir(parents=True, exist_ok=True)
    out_config_path = out_config_dir / "config_plots_v7d_generated.yaml"
