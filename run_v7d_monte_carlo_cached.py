"""
v7d Monte-Carlo runner — CACHED-NETWORK version.
...
Usage:
    python run_v7d_monte_carlo_cached.py \
        --config config/config_debris_v7d.yaml \
        --scenario fema36 --year 2019 \
...
def parse_args():
    ...
    p.add_argument("--config", type=str, default="config/config_debris_v7d.yaml")
    ...
    p.add_argument("--label", type=str, default="v7d")
    return p.parse_args()
...
def main():
    ...
    network_csv = (Path(output_results_folder) /
                   f"result_network_{args.scenario}_{args.year}_{args.label}_"
                   f"seeds{args.start_seed}-{args.start_seed + args.n_samples - 1}.csv")
    summary_csv = (Path(output_results_folder) /
                   f"result_summary_{args.scenario}_{args.year}_{args.label}_"
                   f"seeds{args.start_seed}-{args.start_seed + args.n_samples - 1}.csv")
